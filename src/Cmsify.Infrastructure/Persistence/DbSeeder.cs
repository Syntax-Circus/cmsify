...
public sealed class DbSeeder : IDbSeeder
{
    ...
    public async Task SeedAsync(CancellationToken ct = default)
    {
        ...
        if (!await dbContext.Users.AnyAsync(ct))
        {
            ...
            var passwordHash = configuration["Seed:Admin:PasswordHash"];

            if (string.IsNullOrWhiteSpace(passwordHash))
            {
                throw new InvalidOperationException("Seed:Admin:PasswordHash must be configured before the default admin user can be seeded.");
            }
