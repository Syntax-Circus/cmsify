...
expect(ociLoaderContract?.skopeoImage === "quay.io/skopeo/stable:v1.22.2@sha256:0b98d4296bfd35680c09fd40a5bff17b8569715258a4bee0a7ae3ca500eaaece", "OCI loader Skopeo helper must use the approved immutable versioned tag and linux/amd64 digest.");
...
const artifactApiManifestDigestLine = 'API_MANIFEST_DIGEST="$(jq -er \'.oci.api.digest | select(type == "string" and test("^sha256:[0-9a-f]{64}$"))\' artifacts/release-manifest.json)"';
const artifactAdminManifestDigestLine = 'ADMIN_MANIFEST_DIGEST="$(jq -er \'.oci.admin.digest | select(type == "string" and test("^sha256:[0-9a-f]{64}$"))\' artifacts/release-manifest.json)"';
const artifactApiManifestDigest = artifactSmoke.indexOf(artifactApiManifestDigestLine);
const artifactAdminManifestDigest = artifactSmoke.indexOf(artifactAdminManifestDigestLine);
...
expect(artifactChecksum >= 0 && artifactApiManifestDigest > artifactChecksum && artifactAdminManifestDigest > artifactApiManifestDigest && artifactApiLoad > artifactAdminManifestDigest && artifactAdminLoad > artifactApiLoad && artifactCli > artifactAdminLoad, "Artifact smoke must verify candidate-root checksums, extract both exact checksummed manifest digests, import both exact OCI archives, then invoke the Task 4 CLI.");
expect(/cli\.mjs certify[^\n]*--api-image "docker\.io\/syntaxcircus\/cmsify-api:\$VERSION"[^\n]*--admin-image "docker\.io\/syntaxcircus\/cmsify-admin:\$VERSION"[^\n]*--api-manifest-digest "\$API_MANIFEST_DIGEST"[^\n]*--admin-manifest-digest "\$ADMIN_MANIFEST_DIGEST"[^\n]*--version "\$VERSION"[^\n]*--source-sha "\$SOURCE_SHA"[^\n]*--output "\$RUNNER_TEMP\/cmsify-release-smoke"/.test(artifactSmoke), "Artifact smoke must pass canonical exact loaded images, their certified manifest digests, version, source, and run-owned output identities to Task 4.");
...
expect(/status=\$\?[\s\S]*docker logs --tail 200 cmsify-admin-accessibility[^\n]*\| head -c 262144[^\n]*\|\| true[\s\S]*exit "\$status"/.test(candidateAccessibility) && !/docker logs[^\n]*(?:artifacts|RUNNER_TEMP|GITHUB_WORKSPACE)/.test(candidateAccessibility), "Candidate accessibility failure logs must have fixed line and byte bounds without becoming uploaded evidence.");
...
expect(/TARBALL="\$GITHUB_WORKSPACE\/artifacts\/npm\/syntaxcircus-cmsify-client-\$VERSION\.tgz"/.test(nodeConsumer) && /npm install --ignore-scripts --no-audit --no-fund "\$TARBALL"/.test(nodeConsumer), "Clean Node consumers must install only the downloaded scoped candidate archive filename and its declared registry dependencies.");
...
expect(!/--(?:from-)?oci-layout-path\b/.test(promotion)
  && /oras manifest fetch --descriptor --oci-layout "artifacts\/oci\/api@\$API_EXPECTED"/s.test(promotion)
  && /oras manifest fetch --descriptor --oci-layout "artifacts\/oci\/admin@\$ADMIN_EXPECTED"/s.test(promotion)
  && /oras cp --from-oci-layout "artifacts\/oci\/api@\$API_EXPECTED" "docker\.io\/syntaxcircus\/cmsify-api:\$VERSION"/s.test(promotion)
  && /oras cp --from-oci-layout "artifacts\/oci\/admin@\$ADMIN_EXPECTED" "docker\.io\/syntaxcircus\/cmsify-admin:\$VERSION"/s.test(promotion), "ORAS promotion must use stable local OCI-layout digest syntax instead of experimental path flags.");
