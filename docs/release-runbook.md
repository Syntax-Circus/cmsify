...
The original OCI archive is the certified release artifact and the only promotion source. The loader validates regular non-link archive and manifest paths, the manifest kind/ref/version/digest, and the selected OCI manifest and config blobs before Docker access. Pinned Skopeo 1.22.2 converts that verified candidate offline into run-owned Docker transport scratch with no network or Docker socket access and with the source archive mounted read-only. The pinned Skopeo image is pulled from `ghcr.io/syntax-circus/skopeo`, a mirror this org controls, rather than from `quay.io/skopeo/stable` directly: quay.io repeatedly rotated that tag's manifest digest and garbage-collected the superseded one, breaking the pin every few days. `mirror-skopeo.yml` (`workflow_dispatch`) refreshes the mirror when a new Skopeo version is deliberately adopted; it never runs automatically and never rewrites the pin itself. Docker loads only that transport scratch for `linux/amd64`; the loader then verifies the config digest, ordered DiffIDs, platform, all required OCI labels, and the exact canonical tag against the original archive evidence. The scratch is deleted on success and failure and is never checksummed, uploaded, or promoted. The loader never rebuilds or externally pulls the candidate, and every create-then-fail path performs bounded run-owned cleanup.
