...
The loader validates regular non-link archive and manifest paths, the manifest kind/ref/version/digest, and the selected OCI index descriptor before Docker access. It uses the versioned digest-pinned Registry 2.8.3 and Skopeo 1.22.2 helper images plus two exact labeled run-owned networks: an internal importer and a loopback-published relay. Registry starts on the relay and joins the importer under its run-owned name; it is the only dual-homed helper. Skopeo joins only the internal importer, reads the archive through a read-only mount, and cannot use the relay for external access. The loader never mounts the Docker socket into Skopeo, rebuilds the candidate, or pulls the candidate from an external registry. After the loopback pull it requires the exact certified RepoDigest before exposing the canonical candidate tag, then removes only its intermediate tag, helper containers, and both networks in dependency order. Every create-then-fail path performs the same bounded run-owned cleanup.
