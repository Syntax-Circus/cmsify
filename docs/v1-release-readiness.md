...
- **Evidence:** Cmsify implementation range `29ba5a8^..3cb1855` replaces the manual retry loop with one `HttpRequestResiliencePipeline` for direct, DI, and Admin paths and forwards safe timeout telemetry. The sibling `feature/cmsify-resilience` range `5216a18..017051e` adds the reusable request factory, keyed DI registration, configurable construction-time classifier snapshots shared by retry/circuit logic, bounded retry/timeout/circuit telemetry, best-effort cleanup, ownership rules, and cancellation-first classification/execution precedence. The final local `SyntaxCircus.Http.Resilience` `0.2.0-cmsify.1` package SHA-256 is `F1603F4AFD717117A7133FAABA03B825193604275D97FA69C8F321DBB47A2111`; all five Cmsify asset graphs resolve that exact package/hash as type `package` from the ignored feed with no sibling project path. Fresh validation passed 145 shared-package tests, the direct/keyed/custom-classifier/timeout/canceled-response clean consumer, 66 .NET client tests, 31 Admin tests, and the strict-serial 526-test full solution. The unsuppressed XML-documentation pack emitted 80 warnings (`CS1591` ×75, `CS1573` ×5); a non-incremental Release build retained 45 existing Admin nullable warnings assigned to Task 11.
