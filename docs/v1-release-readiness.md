...
- **Evidence:** Cmsify implementation range `29ba5a8^..3cb1855` replaces the manual retry loop with one `HttpRequestResiliencePipeline` for direct, DI, and Admin paths and forwards safe timeout telemetry. The sibling `feature/cmsify-resilience` range `5216a18..4394b78` adds the reusable request factory, keyed DI registration, configurable construction-time classifier snapshots shared by retry/circuit logic, bounded retry/timeout/circuit telemetry, best-effort cleanup, ownership rules, and regression coverage. The final local `SyntaxCircus.Http.Resilience` `0.2.0-cmsify.1` package SHA-256 is `89762BAF803C33CF9EF802B212032F4A8B227245BD0FE124B81846C06A1C67AC`; all five Cmsify asset graphs resolve that exact package/hash as type `package` from the ignored feed with no sibling project path. Fresh validation passed 142 shared-package tests, the direct/keyed/custom-classifier/timeout clean consumer, 66 .NET client tests, 31 Admin tests, and the strict-serial 526-test full solution. The unsuppressed XML-documentation pack emitted 80 warnings (`CS1591` ×75, `CS1573` ×5); a non-incremental Release build retained 45 existing Admin nullable warnings assigned to Task 11.
