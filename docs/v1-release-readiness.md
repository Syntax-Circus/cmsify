...
- **Score:** Impact 5, Risk 5, Effort 3 → **30**
- **Evidence:** Migration tests create an empty database and verify migrations exist, but no checked-in fixture restores the latest released `0.1.x` database and media state. CI contains no upgrade job. Operations documentation recommends backups and rollback but cannot prove the new image can consume existing state.
