...
Pull requests check out and record the exact event PR head (`pull_request.head.sha`), verify that `git rev-parse HEAD` equals it, then compare its live document against the exact target-branch base (`pull_request.base.sha`). Pushes compare the live document at the pushed commit against the event's exact previous tip (`before`); a newly-created branch falls back to the pushed commit's first parent. The breaking comparison uses oasdiff `1.28.0` and is scoped to `/api/v1`.
