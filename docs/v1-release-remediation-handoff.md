...
- Public/CI restore for the full default-branch solution remains blocked until the sibling work is merged, `SyntaxCircus.Http.Resilience` `0.2.0-cmsify.1` is packed and approved from its default branch, any changed identity/locks are reconciled into Cmsify, and those approved bytes are published through the trusted workflow. Pull requests run the public-independent Core, Infrastructure, API, OpenAPI, and TypeScript checks while explicitly deferring package-dependent Admin/.NET-client and Admin accessibility coverage; that partial result is not public-package evidence. The current branch-built local artifact must not be published. A separately approved replacement is an alternative only after its package identity, version, content hash, central pin, and lock entries are reviewed; the ordinary locked restore and all downstream evidence must then be rerun.
