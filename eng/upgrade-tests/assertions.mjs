...
import { requestBytes, requestJson } from "./http.mjs";
...
function quote(value) {
  ensure(typeof value === "string" && value.length > 0, "an assertion SQL value is missing");
...
function dockerSqlAdapter(docker) {
  ...
  return Object.freeze({
    ...
    async json(statement) {
      const value = await scalar(statement);
      try {
...
function sqlOf(context) {
  return context.sql ?? dockerSqlAdapter(context.docker);
}

async function scalar(context, statement) {
  const adapter = sqlOf(context);
  ensure(typeof adapter.scalar === "function", "a SQL scalar adapter is required");
  return adapter.scalar(statement);
}

async function jsonRows(context, statement) {
  const adapter = sqlOf(context);
  ensure(typeof adapter.json === "function", "a SQL JSON adapter is required");
  return adapter.json(statement);
}

async function expectSqlCount(context, statement, expected, message) {
  ensure(await scalar(context, statement) === String(expected), message);
}
...
async function assertHealthReady(context) {
  ...
  if (context.phase === "candidate") ensure(response.body?.metadata?.version === context.candidate?.version, "candidate readiness version does not match the tested image version");
}
...
async function assertWorkspaces(context) {
  ...
  await expectSqlCount(context, `SELECT count(*) FROM workspaces WHERE id IN ('${quote(ids.primaryWorkspace)}', '${quote(ids.restrictedWorkspace)}') AND created_at = '${quote(timestamps.workspaceCreatedAt)}'::timestamptz AND updated_at = '${quote(timestamps.workspaceUpdatedAt)}'::timestamptz;`, 2, "both fixed synthetic workspaces must remain related and timestamp-stable");
}

async function assertEditorGrant(context) {
  const ids = idsOf(context);
  await expectSqlCount(context, `SELECT count(*) FROM user_workspace_accesses access JOIN users actor ON actor.id = access.user_id JOIN workspaces workspace ON workspace.id = access.workspace_id WHERE access.user_id = '${quote(ids.editorUser)}' AND access.workspace_id = '${quote(ids.primaryWorkspace)}' AND access.access_level = 'Write';`, 1, "the editor write grant and both foreign-key parents must remain linked");
}
...
async function assertTemplateFields(context) {
  ...
  await expectSqlCount(context, `SELECT count(*) FROM template_fields field JOIN template_versions version ON version.id = field.template_version_id JOIN templates template ON template.id = version.template_id WHERE field.id IN ('${quote(ids.titleField)}', '${quote(ids.choiceField)}', '${quote(ids.componentField)}') AND version.id = '${quote(ids.templateVersion)}' AND template.id = '${quote(ids.template)}' AND template.created_at = '${quote(context.expected.timestamps.templateCreatedAt)}'::timestamptz AND template.updated_at = '${quote(context.expected.timestamps.templateUpdatedAt)}'::timestamptz;`, 3, "template fields, foreign-key parents, and fixed timestamps must remain linked");
}
...
async function assertComponentSnapshot(context) {
  ...
  await expectSqlCount(context, `SELECT count(*) FROM components WHERE id = '${quote(ids.component)}' AND created_at = '${quote(context.expected.timestamps.componentCreatedAt)}'::timestamptz AND updated_at = '${quote(context.expected.timestamps.componentUpdatedAt)}'::timestamptz;`, 1, "component fixed timestamps changed");
  await expectSqlCount(context, `SELECT count(*) FROM component_fields field JOIN component_versions version ON version.id = field.component_version_id JOIN components component ON component.id = version.component_id WHERE version.id = '${quote(ids.componentVersion)}' AND component.id = '${quote(ids.component)}' AND field.nested_component_id IS NOT NULL;`, 0, "component graph must remain acyclic in storage");
}
...
async function assertImmutableRevisions(context) {
  ...
  const timestamps = context.expected.timestamps;
  await expectSqlCount(context, `SELECT count(*) FROM pick_list_revisions revision JOIN pick_lists choice_set ON choice_set.id = revision.pick_list_id WHERE revision.pick_list_id = '${quote(ids.choiceSet)}' AND ((revision.id = '${quote(ids.choiceRevisionOne)}' AND revision.version_number = 1 AND revision.created_at = '${quote(timestamps.choiceRevisionOneCreatedAt)}'::timestamptz) OR (revision.id = '${quote(ids.choiceRevisionTwo)}' AND revision.version_number = 2 AND revision.created_at = '${quote(timestamps.choiceRevisionTwoCreatedAt)}'::timestamptz));`, 2, "both immutable choice revisions and fixed timestamps must remain intact");
}
...
async function assertPublishedChoiceLabel(context) {
  ...
  await expectSqlCount(context, `SELECT count(*) FROM content_version_field_values value JOIN content_versions version ON version.id = value.content_version_id WHERE value.content_version_id = '${quote(ids.publishedVersion)}' AND value.field_id = '${quote(ids.choiceField)}' AND value.text_value = '${quote(context.expected.content.publishedChoiceValue)}' AND value.display_label = '${quote(context.expected.content.publishedChoiceLabel)}';`, 1, "published choice value and label snapshot must remain immutable");
}
...
async function assertContentVersions(context) {
  ...
  await expectSqlCount(context, `SELECT count(*) FROM content_items content JOIN template_versions template_version ON template_version.id = content.template_version_id JOIN workspaces workspace ON workspace.id = content.workspace_id WHERE content.id IN ('${quote(ids.draftContent)}', '${quote(ids.publishedContent)}') AND template_version.id = '${quote(ids.templateVersion)}' AND workspace.id = '${quote(ids.primaryWorkspace)}' AND ((content.id = '${quote(ids.draftContent)}' AND content.created_at = '${quote(context.expected.timestamps.draftContentCreatedAt)}'::timestamptz AND content.updated_at = '${quote(context.expected.timestamps.draftContentCreatedAt)}'::timestamptz) OR (content.id = '${quote(ids.publishedContent)}' AND content.created_at = '${quote(context.expected.timestamps.publishedContentCreatedAt)}'::timestamptz AND content.updated_at = '${quote(context.expected.timestamps.publishedContentUpdatedAt)}'::timestamptz));`, 2, "draft and published content relationships and fixed timestamps must remain distinct");
}
...
async function assertFuturePublishAt(context) {
  ...
  await expectSqlCount(context, `SELECT count(*) FROM content_items WHERE id = '${quote(ids.scheduledContent)}' AND status = 'Approved' AND publish_at = '${quote(context.expected.content.scheduledPublishAt)}'::timestamptz AND created_at = '${quote(context.expected.timestamps.scheduledContentCreatedAt)}'::timestamptz AND updated_at = '${quote(context.expected.timestamps.scheduledContentUpdatedAt)}'::timestamptz;`, 1, "future PublishAt and fixed scheduled timestamps must remain intact");
}
...
async function assertBoundedCurrent(context) {
  ...
  await expectSqlCount(context, `SELECT count(*) FROM content_versions WHERE id = '${quote(ids.publishedVersion)}' AND effective_start_at = '${quote(context.expected.content.currentEffectiveStartAt)}'::timestamptz AND effective_end_at = '${quote(context.expected.content.currentEffectiveEndAt)}'::timestamptz;`, 1, "bounded currently-effective range must remain intact");
}
...
async function assertExpiredRange(context) {
  ...
  await expectSqlCount(context, `SELECT count(*) FROM content_versions version JOIN content_items content ON content.id = version.content_item_id WHERE version.id = '${quote(ids.expiredVersion)}' AND version.effective_start_at = '${quote(context.expected.content.expiredEffectiveStartAt)}'::timestamptz AND version.effective_end_at = '${quote(context.expected.content.expiredEffectiveEndAt)}'::timestamptz AND content.created_at = '${quote(context.expected.timestamps.expiredContentCreatedAt)}'::timestamptz AND content.updated_at = '${quote(context.expected.timestamps.expiredContentUpdatedAt)}'::timestamptz;`, 1, "expired effective range and fixed content timestamps must remain intact");
}
...
async function assertAvailableMedia(context) {
  ...
  await expectSqlCount(context, `SELECT count(*) FROM media_assets WHERE id = '${quote(ids.textMedia)}' AND NOT is_deleted AND deleted_at IS NULL AND storage_provider = '${quote(context.expected.candidate.storageProvider)}' AND storage_key = '${quote(expectedMedia.storageKey)}' AND created_at = '${quote(context.expected.timestamps.textMediaCreatedAt)}'::timestamptz AND updated_at = '${quote(context.expected.timestamps.textMediaCreatedAt)}'::timestamptz;`, 1, "available media canonical storage identity and fixed timestamps changed");
}
...
async function assertDeletedMediaHidden(context) {
  ...
  await expectSqlCount(context, `SELECT count(*) FROM media_assets WHERE id = '${quote(ids.imageMedia)}' AND is_deleted AND deleted_at = '${quote(expectedMedia.lifecycle.historicalDeletedAt)}'::timestamptz AND file_name = '${quote(expectedMedia.fileName)}' AND mime_type = '${quote(expectedMedia.contentType)}' AND size_bytes = ${expectedMedia.sizeBytes} AND storage_key = '${quote(expectedMedia.storageKey)}' AND created_at = '${quote(context.expected.timestamps.imageMediaCreatedAt)}'::timestamptz AND updated_at = '${quote(context.expected.timestamps.imageMediaUpdatedAt)}'::timestamptz;`, 1, "historical soft-deleted media metadata and fixed deletion boundary changed");
...
async function assertCandidateDeletionBoundary(context) {
  ...
  if (context.phase !== "candidate") {
    await expectSqlCount(context, `SELECT count(*) FROM media_assets WHERE (id = '${quote(ids.textMedia)}' AND NOT is_deleted AND deleted_at IS NULL) OR (id = '${quote(ids.imageMedia)}' AND is_deleted AND deleted_at = '${quote(context.expected.media.image.lifecycle.historicalDeletedAt)}'::timestamptz);`, 2, "historical active and deleted media lifecycle boundary changed");
    return;
  }
  const rows = await jsonRows(context, `SELECT COALESCE(jsonb_agg(jsonb_build_object('id', asset.id, 'provider', asset.storage_provider, 'storageKey', asset.storage_key, 'blobState', asset.blob_state, 'deletionIntentReason', intent.reason, 'deletionIntentProvider', intent.provider, 'deletionIntentStorageKey', intent.storage_key, 'deletionIntentCount', (SELECT count(*) FROM media_deletion_intents all_intents WHERE all_intents.media_asset_id = asset.id)) ORDER BY asset.id), '[]'::jsonb)::text FROM media_assets asset LEFT JOIN media_deletion_intents intent ON intent.media_asset_id = asset.id AND intent.completed_at IS NULL WHERE asset.id IN ('${quote(ids.textMedia)}', '${quote(ids.imageMedia)}');`);
...
async function assertInertWebhook(context) {
  ...
  await expectSqlCount(context, `SELECT count(*) FROM webhook_endpoints WHERE id = '${quote(ids.webhook)}' AND NOT is_active AND NOT is_deleted AND created_at = '${quote(context.expected.timestamps.webhookCreatedAt)}'::timestamptz AND updated_at = '${quote(context.expected.timestamps.webhookCreatedAt)}'::timestamptz;`, 1, "fixture webhook must remain inactive with fixed timestamps");
}
...
async function assertTerminalDelivery(context) {
  ...
  await expectSqlCount(context, `SELECT count(*) FROM webhook_delivery_logs delivery JOIN webhook_endpoints endpoint ON endpoint.id = delivery.webhook_endpoint_id WHERE delivery.id = '${quote(ids.webhookDelivery)}' AND endpoint.id = '${quote(ids.webhook)}' AND delivery.attempt_count = 10 AND delivery.is_failed AND NOT delivery.is_delivered AND delivery.next_retry_at IS NULL AND delivery.lease_expires_at IS NULL AND delivery.created_at = '${quote(context.expected.timestamps.webhookDeliveryCreatedAt)}'::timestamptz AND delivery.last_attempt_at = '${quote(context.expected.timestamps.webhookDeliveryLastAttemptAt)}'::timestamptz;`, 1, "terminal webhook delivery history and fixed timestamps changed");
}

async function webhookWorkerState(context) {
  const ids = idsOf(context);
  return scalar(context, `SELECT jsonb_build_object('endpointActive', endpoint.is_active, 'attemptCount', delivery.attempt_count, 'lastAttemptAt', delivery.last_attempt_at, 'nextRetryAt', delivery.next_retry_at, 'statusCode', delivery.status_code, 'isDelivered', delivery.is_delivered, 'isFailed', delivery.is_failed, 'leaseExpiresAt', delivery.lease_expires_at)::text FROM webhook_endpoints endpoint JOIN webhook_delivery_logs delivery ON delivery.webhook_endpoint_id = endpoint.id WHERE endpoint.id = '${quote(ids.webhook)}' AND delivery.id = '${quote(ids.webhookDelivery)}';`);
}
...
async function assertAuthentication(context) {
  ...
  await expectSqlCount(context, `SELECT count(*) FROM api_clients WHERE id = '${quote(ids.readerClient)}' AND role = 'Reader' AND workspace_id = '${quote(ids.primaryWorkspace)}' AND is_active AND token_identifier = '${quote(context.expected.authentication.readerTokenIdentifier)}' AND created_at = '${quote(context.expected.timestamps.readerClientCreatedAt)}'::timestamptz;`, 1, "fixture reader API client metadata changed");
}
...
async function assertPackageProvenance(context) {
  const ids = idsOf(context);
  const provenance = context.expected.provenance;
  await expectSqlCount(context, `SELECT (SELECT count(*) FROM components WHERE id = '${quote(ids.component)}' AND package_namespace = '${quote(provenance.packageNamespace)}' AND package_id = '${quote(provenance.packageId)}' AND package_version = '${quote(provenance.packageVersion)}') + (SELECT count(*) FROM pick_lists WHERE id = '${quote(ids.choiceSet)}' AND package_namespace = '${quote(provenance.packageNamespace)}' AND package_id = '${quote(provenance.packageId)}' AND package_version = '${quote(provenance.packageVersion)}') + (SELECT count(*) FROM templates WHERE id = '${quote(ids.template)}' AND package_namespace IS NULL AND package_id IS NULL AND package_version IS NULL);`, 3, "component, choice-set, and template package provenance columns changed");
}
...
async function assertLegacyWebhookReadable(context) {
  const ids = idsOf(context);
  const ciphertext = await scalar(context, `SELECT secret FROM webhook_endpoints WHERE id = '${quote(ids.webhook)}';`);
  let actualSha;
  try {
...
async function assertCanaryWriteRead(context) {
  ...
  const created = await httpJson(context, `/api/v1/workspaces/${ids.primaryWorkspace}/content`, {
    ...
    body: {
      ...
      fields: [
        { fieldId: ids.titleField, order: 0, valueKind: "Text", textValue: `Upgrade canary ${context.runId ?? "rehearsal"}`, jsonValue: null, ...nullable },
...
function parseDockerJsonResponse(stdout, method, path, expectedStatuses) {
  const normalized = stdout.replaceAll("\r\n", "\n");
  const separator = normalized.indexOf("\n\n");
  ensure(separator >= 0, `HTTP ${method} ${path} failed (malformed response); response body was withheld`);
  const headerLines = normalized.slice(0, separator).split("\n");
  const match = /^HTTP\/\S+\s+(\d{3})\b/.exec(headerLines[0]);
  ensure(match, `HTTP ${method} ${path} failed (missing status); response body was withheld`);
  ...
  ensure(expectedStatuses.has(status), `HTTP ${method} ${path} failed; status ${status}; response body was withheld`);
  let body;
  if (bodyText.length > 0) {
    try { body = JSON.parse(bodyText); } catch { throw new Error(`HTTP ${method} ${path} failed (invalid JSON); status ${status}; response body was withheld`); }
  }
  return Object.freeze({ status, headers, body });
}

function dockerHttpAdapter(docker, service) {
  ensure(docker && typeof docker.exec === "function", "a Docker HTTP adapter is required");
  async function execute(args, method, path) {
    try { return await docker.exec(service, ["curl", ...args]); } catch { throw new Error(`HTTP ${method} ${path} failed (Docker transport error); authorization and response body were withheld`); }
  }
  return Object.freeze({
    async requestJson(request) {
      const url = new URL(request.url);
      const method = request.method ?? "GET";
      const args = ["--silent", "--show-error", "--max-time", "5", "--max-redirs", "0", "--include", "--request", method];
      if (request.token) args.push("--header", `Authorization: Bearer ${request.token}`);
      if (request.body !== undefined) args.push("--header", "Content-Type: application/json", "--data", JSON.stringify(request.body));
      args.push(url.toString());
      const result = await execute(args, method, url.pathname);
      return parseDockerJsonResponse(result.stdout, method, url.pathname, request.expectedStatuses);
    },
    async requestBytes(request) {
      const url = new URL(request.url);
      const method = request.method ?? "GET";
      const destination = `/tmp/cmsify-http-${createHash("sha256").update(url.pathname).digest("hex").slice(0, 16)}`;
      const args = ["--silent", "--show-error", "--max-time", "5", "--max-redirs", "0", "--request", method, "--output", destination, "--write-out", "%{http_code}"];
      if (request.token) args.push("--header", `Authorization: Bearer ${request.token}`);
      args.push(url.toString());
      const result = await execute(args, method, url.pathname);
      const status = Number(result.stdout.trim());
      ensure(request.expectedStatuses.has(status), `HTTP ${method} ${url.pathname} failed; status ${status}; response body was withheld`);
      try {
        const [digestResult, sizeResult] = await Promise.all([docker.exec(service, ["sha256sum", destination]), docker.exec(service, ["wc", "-c", destination])]);
        return Object.freeze({ status, headers: new Headers(), bytes: Buffer.alloc(0), byteLength: Number(sizeResult.stdout.trim().split(/\s+/, 1)[0]), sha256: digestResult.stdout.trim().split(/\s+/, 1)[0] });
      } catch {
        throw new Error(`HTTP ${method} ${url.pathname} byte verification failed; response body was withheld`);
      }
    },
  });
}
...
export async function assertBaselineFixture({ harness, expected, ids, webhookWorkerStateBeforeStart, fixture }) {
  ...
  await assertBaseline({
    ...
    http: dockerHttpAdapter(harness, "baseline-api"), sql: dockerSqlAdapter(harness), storage: dockerStorageAdapter(harness),
