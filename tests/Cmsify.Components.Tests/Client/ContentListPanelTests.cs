...
public sealed class ContentListPanelTests : BunitContext
{
    ...
    public void RaisesOnViewWhenViewButtonClicked()
    {
        ...
        cut.WaitForState(() => cut.FindAll(".cmsify-list-view-button").Count > 0);

        cut.Find(".cmsify-list-view-button").Click();

        cut.WaitForState(() => viewed is not null);

        viewed.ShouldNotBeNull();
        viewed!.Id.ShouldBe(contentId);
    }
    ...
    public void InitialLoadFetchesTemplatesAndContentConcurrently()
    {
        ...
        // If the templates and content fetches ran one at a time, the gate would never see more
        // than one waiting at once. Observing 2 proves they were fired concurrently.
