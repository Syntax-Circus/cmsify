...
test("rejects combined ORAS boolean and path flags", () => expectInvalid((root) => mutateWorkflow(root, (workflow) => workflow.replace('oras manifest fetch --descriptor --oci-layout "artifacts/oci/api@$API_EXPECTED"', 'oras manifest fetch --descriptor --oci-layout --oci-layout-path artifacts/oci/api "docker.io/syntaxcircus/cmsify-api@$API_EXPECTED"')), /ORAS.*combined.*--oci-layout/i));
test("rejects combined ORAS copy boolean and path flags", () => expectInvalid((root) => mutateWorkflow(root, (workflow) => workflow.replace('oras cp --from-oci-layout "artifacts/oci/api@$API_EXPECTED"', 'oras cp --from-oci-layout --from-oci-layout-path artifacts/oci/api "docker.io/syntaxcircus/cmsify-api@$API_EXPECTED"')), /ORAS.*combined.*--from-oci-layout/i));
test("rejects experimental ORAS path syntax for exact local-layout digests", () => expectInvalid(
  (root) => mutateWorkflow(root, (workflow) => workflow
    .replace('oras manifest fetch --descriptor --oci-layout "artifacts/oci/api@$API_EXPECTED"', 'oras manifest fetch --descriptor --oci-layout-path artifacts/oci/api "docker.io/syntaxcircus/cmsify-api@$API_EXPECTED"')
    .replace('oras cp --from-oci-layout "artifacts/oci/api@$API_EXPECTED"', 'oras cp --from-oci-layout-path artifacts/oci/api "docker.io/syntaxcircus/cmsify-api@$API_EXPECTED"')),
  /ORAS.*stable.*local OCI-layout.*digest/i,
));
...
test("rejects artifact smoke without the API certified manifest digest", () => expectInvalid((root) => mutateReleaseJob(root, "artifact-smoke", (job) => job.replace(/\s*--api-manifest-digest "\$API_MANIFEST_DIGEST"/, "")), /artifact smoke.*manifest digest/i));
test("rejects artifact smoke without the Admin certified manifest digest", () => expectInvalid((root) => mutateReleaseJob(root, "artifact-smoke", (job) => job.replace(/\s*--admin-manifest-digest "\$ADMIN_MANIFEST_DIGEST"/, "")), /artifact smoke.*manifest digest/i));
test("rejects artifact smoke with swapped certified manifest digests", () => expectInvalid((root) => mutateReleaseJob(root, "artifact-smoke", (job) => job
  .replace('--api-manifest-digest "$API_MANIFEST_DIGEST"', '--api-manifest-digest "$ADMIN_MANIFEST_DIGEST"')
  .replace('--admin-manifest-digest "$ADMIN_MANIFEST_DIGEST"', '--admin-manifest-digest "$API_MANIFEST_DIGEST"')), /artifact smoke.*manifest digest/i));
test("rejects manifest digest extraction before candidate checksum verification", () => expectInvalid((root) => mutateReleaseJob(root, "artifact-smoke", (job) => {
  const checksum = "          (cd artifacts && sha256sum --check SHA256SUMS)\n";
  const api = job.match(/^\s*API_MANIFEST_DIGEST=.*$/m)?.[0];
  const admin = job.match(/^\s*ADMIN_MANIFEST_DIGEST=.*$/m)?.[0];
  return job.replace(`${checksum}${api}\n${admin}\n`, `${api}\n${admin}\n${checksum}`);
}), /artifact smoke.*checksummed.*manifest digest|artifact smoke.*manifest digest.*checksum/i));
...
test("rejects candidate accessibility failure logs without a line bound", () => expectInvalid((root) => mutateReleaseJob(root, "candidate-accessibility", (job) => job.replace("docker logs --tail 200", "docker logs")), /candidate accessibility.*logs.*bound/i));
test("rejects candidate accessibility failure logs without a byte bound", () => expectInvalid((root) => mutateReleaseJob(root, "candidate-accessibility", (job) => job.replace(" | head -c 262144", "")), /candidate accessibility.*logs.*bound/i));
