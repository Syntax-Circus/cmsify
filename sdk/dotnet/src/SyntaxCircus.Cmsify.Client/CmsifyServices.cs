...
public sealed class ContentClient(CmsifyClient client)
{
    ...
    // expandChildren=true (the default) preserves the exact URL/response shape every existing caller
    // already gets. Editing code (ContentEditPanel/ContentEditSupport) only ever reads
    // ChildContentItemId, never the nested Child response, so it passes false to skip the server's
    // (now-batched, but still not free) recursive child expansion entirely - see ContentController's
    // ToVersionDetailResponseAsync.
    public Task<ContentVersionDetailResponse?> CreateVersionAsync(Guid workspaceId, Guid id, CreateContentVersionRequest request, CancellationToken ct = default, bool expandChildren = true) => client.PostAsync<ContentVersionDetailResponse>(CmsifyClient.WorkspacePath(workspaceId, $"/content/{id}/versions{Query.ExpandChildren(expandChildren)}"), request, ct);
    ...
    public Task<ContentVersionDetailResponse?> GetVersionAsync(Guid workspaceId, Guid id, int versionNumber, CancellationToken ct = default, bool expandChildren = true) => client.GetAsync<ContentVersionDetailResponse>(CmsifyClient.WorkspacePath(workspaceId, $"/content/{id}/versions/{versionNumber}{Query.ExpandChildren(expandChildren)}"), ct);
    public Task<ContentVersionDetailResponse?> UpdateVersionAsync(Guid workspaceId, Guid id, int versionNumber, UpdateContentVersionRequest request, CancellationToken ct = default, bool expandChildren = true) => client.PutAsync<ContentVersionDetailResponse>(CmsifyClient.WorkspacePath(workspaceId, $"/content/{id}/versions/{versionNumber}{Query.ExpandChildren(expandChildren)}"), request, ct);
...
internal static class Query
{
    ...
    public static string ExpandChildren(bool expandChildren) => expandChildren ? string.Empty : "?expandChildren=false";
